...
<body>
		...
		<c:if test="${empty etudiants }">
			...
			<img src="img/empty-state.svg" alt="Empty state illustration">
		  	<h4>Les Résultats de préselections ne sont pas encore sorties, Veuillez respectez la date de publication mentionée.</h4>
		</div>
		</c:if>
		...
		<c:if test="${!empty etudiants }">
			...
			<div class="footer">
		   		<div>
					<span> ${master.nom_uni}</span>
					<span >${master.nom_fac} </span>
					</div>
			   		<span>${ datePub }</span>
		  		</div>
		  </div>
